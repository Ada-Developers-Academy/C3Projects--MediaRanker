RSpec.shared_examples "MediaController" do
    ...
    describe "POST #create" do
      ...
      context "valid media params" do
        ...
        it "redirects to the show page" do
          post :create, @media_params
          expect(subject).to redirect_to(book_path(assigns(@media)))
        end
      end
    ...
    describe "POST #upvote" do
      before :each do 
        @model = @model.create(name: "name", vote: 0)
      end

      context "clicking upvote button" do
        it "increases the upvote count by 1" do
          post :upvote, id: @model.id
          @model.reload
          expect(@model.vote).to eq 1
        end

        it "redirects to the show page" do
          patch :upvote, id: @model.id
          @model.reload
          expect(subject).to redirect_to(book_path)
        end
      end
